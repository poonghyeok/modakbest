...
<body>	
		<c:if test ="${!empty classList }">
 		...
 		<select name="user_classid" class="form-control input-sm" id="user_classid"> 
			<option selected>학원명</option>
			<c:forEach items="${classList }" var="classList">
				<option value="${classList.class_academy}" data-landURL="${classList.class_id}"></option>
			</c:forEach>
		</select> 
		...
		<div>
		  ...
		  <input type="text" list="list" id="numbers" style="margin-bottom: 10px;"/>
		  <datalist id ="list">
		    <option value="1" />
		    <option value="2" />
		    <option value="3" />
...
/* $(function(){
    $.ajax({
        type: 'post',
        url: '/semiproject/user/getClassList',
        dataType: 'json',
